...
function showProfileDetails() {
    ...
    const databaseRef1 = firebase.database().ref('/users/' + id);
    ...
    databaseRef1.once('value').then(function(snapshot) {
        ...
        console.log(jobData);
        var jobDetailsDiv = document.getElementById('profile-details');

        // Display job details
        jobDetailsDiv.innerHTML = `
            <p>Username: ${jobData.username || jobData.producerUsername}</p>
            <p>Phone no.: <a href="tel:${jobData.phone}">${jobData.phone}</a></p>
